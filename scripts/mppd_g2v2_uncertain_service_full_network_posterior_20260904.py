...
def load_uncertain_service(path):
    ...
    return roots,payload.get('manifest',{})
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--p1c',required=True);ap.add_argument('--cohorts',required=True);ap.add_argument('--routes',required=True);ap.add_argument('--service-init',required=True);ap.add_argument('--out',required=True);ap.add_argument('--beam',type=int,default=12);args=ap.parse_args()
    ...
    t=time.perf_counter();G,meta,code_to_nodes,transfer_groups,ambiguous_seq,ambiguous_codes,graph_build=g0.build_network(args.p1c);phase['network_build_sec']=time.perf_counter()-t
    t=time.perf_counter();routes=support.load_routes(args.routes);phase['route_cache_load_sec']=time.perf_counter()-t
    t=time.perf_counter();roots,svc_manifest=load_uncertain_service(args.service_init);rides_fn,_=build_uncertain_rides(roots);phase['service_cache_load_sec']=time.perf_counter()-t
    ...
    result={'schema':'mppd.g2v2-uncertain-service-full-network-posterior.v1','date':'2026-09-04','status':'G2V2_UNCERTAINTY_AWARE_FULL_NETWORK_POSTERIOR_COMPLETED','authority':'00_CURRENT_CORE_CLOSURE_WORKPLAN_V6_FULL_NETWORK_STATE_RECONSTRUCTION_20260904.md',

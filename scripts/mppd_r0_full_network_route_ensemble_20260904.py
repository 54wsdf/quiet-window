...
def weight_fn(pol,meta):
    def w(u,v,d):
        ...
        if kind in ('transfer','transfer_repaired'):mult*=pol['transfer']
        elif kind=='inline_uncertain':mult*=pol['uncertain']
        seed=pol.get('seed')
        if seed is not None:
            key='|'.join(sorted((u,v)))+f'|{seed}'
            q=hunit(key)
            if kind in ('transfer','transfer_repaired'):mult*=0.65+0.9*q
            else:
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--p1c',required=True);ap.add_argument('--cohorts',required=True);ap.add_argument('--out',required=True);ap.add_argument('--topology-patch');args=ap.parse_args()
    ...
    base_components=nx.number_connected_components(G);base_edges=G.number_of_edges();patch_result=None
    if args.topology_patch:
        patch=load_patch(args.topology_patch);patch_result=apply_topology_patch(G,meta,patch)
    topology_state={'base_edges':base_edges,'base_connected_components':base_components,'patched_edges':G.number_of_edges(),'patched_connected_components':nx.number_connected_components(G),'topology_patch_applied':bool(args.topology_patch),'topology_patch':patch_result}
    ...
    with gzip.open(outdir/'r0_full_network_route_ensemble_candidates.jsonl.gz','wt',encoding='utf-8') as f:
        for od,mass in sorted(od_mass.items()):
            ...
            f.write(json.dumps({'origin_code':od[0],'destination_code':od[1],'passenger_mass':mass,'candidate_set_status':'ENSEMBLE_STRUCTURAL_HYPOTHESES','topology_patch_applied':bool(args.topology_patch),'candidates':cands},ensure_ascii=False)+'\n')
    result={'schema':'mppd.r0-full-network-route-ensemble.v2','date':'2026-09-04','status':'R0_FULL_NETWORK_ROUTE_ENSEMBLE_COMPLETED','authority':'00_CURRENT_CORE_CLOSURE_WORKPLAN_V6_FULL_NETWORK_STATE_RECONSTRUCTION_20260904.md',
      ...
      'performance':{'wall_sec':time.perf_counter()-wall0},'scientific_boundary':['Ensemble paths are structural route hypotheses, not observed routes and not guaranteed to exhaust all near-optimal alternatives.','No transfer-count cap is applied.','An explicit topology patch, when supplied, remains provenance-typed and does not become observed route or ATS truth.','The ensemble is an efficient candidate-diversity substrate; KSP shards remain the deeper alternative-set audit.'],
      'next_gate':'Use patched candidate-count and structural-coverage gains to qualify the routing substrate, then rerun uncertainty-aware G2v2 before G3.','no_email_notification_logic':True}

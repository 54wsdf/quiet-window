...
TRANSFER_MOVEMENT_MIN_WEIGHT = 50.0
...
def interval_fit(hist, initial_kernel, min_weight, evidence_class):
    ...
    if total < min_weight:
        return initial_kernel, {'fitted': False, 'effective_weight': total, 'unique_signature_count': len(hist)}
    ...
    return base.kernel_from_median_sigma(med, sig, evidence_class), {'fitted': bool(fit.success), 'effective_weight': total, 'unique_signature_count': len(rows), 'median_sec': med, 'sigma': sig, 'objective_per_mass': float(fit.fun)}
...
def point_fit(hist, initial_kernel, min_weight, evidence_class):
    ...
    return base.kernel_from_median_sigma(med, sig, evidence_class), {'fitted': True, 'effective_weight': total, 'unique_signature_count': len(rows), 'median_sec': med, 'sigma': sig}
...
def main():
    ...
    hists, files = load_stats(args.stats_root); prior = default_kernels()
    ...
    for (scope, group), hist in hists.items():
        if scope != 'TRANSFER_MOVEMENT' or group is None: continue
        raw, d = interval_fit(hist, raw_k, TRANSFER_MOVEMENT_MIN_WEIGHT, 'R1C_FULL_RAW_TRANSFER_MOVEMENT_K1')
        if not d.get('fitted') or d.get('effective_weight',0.0) < TRANSFER_MOVEMENT_MIN_WEIGHT: continue
        transfer_by_movement[group] = v2.blend_kernel(transfer_global, raw, GLOBAL_DAMPING, 'R1C_FULL_DAMPED_TRANSFER_MOVEMENT_K1')
        transfer_diag[group] = d
    ...
    kernels = {
        'schema':'mppd.r1c-full-denominator-hierarchical-temporal-kernels.v1-k1-mixture-capable',
    ...
    diagnostics = {
        'schema':'mppd.r1c-full-denominator-kernel-aggregation-diagnostics.v1','date':'2026-09-05','source_shard_file_count':len(files),
        'global':{'access':diag_a,'transfer':diag_k,'egress':diag_e,'damping':GLOBAL_DAMPING},
        'counts':{'access_context_count':len(access_by_context),'access_context_hour_count':len(access_by_context_hour),'egress_context_count':len(egress_by_context),'egress_context_hour_count':len(egress_by_context_hour),'transfer_movement_count':len(transfer_by_movement)},
        'hyperparameters':{'context_min_weight':CONTEXT_MIN_WEIGHT,'context_shrink_tau':CONTEXT_SHRINK_TAU,'hour_min_weight':HOUR_MIN_WEIGHT,'hour_shrink_tau':HOUR_SHRINK_TAU,'transfer_movement_min_weight':TRANSFER_MOVEMENT_MIN_WEIGHT},
        'access_context':access_context_diag,'access_context_hour':access_hour_diag,'egress_context':egress_context_diag,'egress_context_hour':egress_hour_diag,'transfer_movement':transfer_diag,
        'scientific_boundary':['All input records are shard-local posterior sufficient statistics and are merged by additive posterior weight before fitting.','Global damping and hierarchical shrinkage exactly retain the declared R1C first-pass hyperparameters; no full-denominator retuning is performed.','K=1 remains a first-pass representation and all serialized kernels remain mixture-capable.'],
